...
def qwen3_vl_moe_sparse_moe_block_forward(
    self: "Qwen3VLMoeTextSparseMoeBlock", hidden_states: torch.Tensor
) -> torch.Tensor:
    """Patched forward pass to ensure dtype consistency for router weights."""
    ...
    router_weights = torch.zeros_like(router_logits, dtype=routing_weights.dtype)
    router_weights.scatter_(1, router_indices, routing_weights)
