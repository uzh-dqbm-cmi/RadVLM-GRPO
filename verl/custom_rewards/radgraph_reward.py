...
def radgraph_score(pred_text, true_text, retries=3, timeout=5):
    backoff = 0.1

    for attempt in range(retries + 1):
        url = f"{_next_endpoint()}/score_radgraph"
        try:
            r = requests.post(
                url,
...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == "mimic_grpo":
        return radgraph_score(solution_str, ground_truth)
        ...
        if i == -1:
            ...
            return radgraph_score(core_answer, ground_truth)
