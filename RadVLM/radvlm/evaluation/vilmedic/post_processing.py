...
def plot_attention(
    results, pp_dir, seed, logger, split, epoch, dl, smooth=True, **kwargs
):
    ...
    for sample in dataset:
        ...
        if smooth:
            alpha_im = skimage.transform.pyramid_expand(
                attentions[-1, -1, -1, -1, -1, :].reshape(square, square),
                upscale=int(width / square),
                sigma=20,
            )
        else:
            alpha_im = skimage.transform.resize(
                attentions[-1, -1, -1, -1, -1, :].reshape(square, square),
                [width, height],
            )
...
def save_representation(keys, results, pp_dir, seed, logger, split, **kwargs):
    ...
    for key in keys:
        ...
        results.pop(key)
        np.save(
            os.path.join(out_dir, split + "_" + str(key) + "_" + "embeddings"),
            embeddings,
        )
...
def plot_representation(
    ...
    **kwargs
    ...
    for key in keys:
        ...
        np.save(
            os.path.join(out_dir, split + "_" + str(key) + "_" + "labels"), emb_labels
        )

        assert len(embeddings) != 0, logging.error(
            "No embedding kept for visualization"
        )
        ...
        if max_samples_per_class is not None:
            if not isinstance(max_samples_per_class, int):
                logger.warn(
                    "Argument max_samples_per_class is not an integer, found {}. Using all points".format(
        ...
        for visualization in [
            TSNE(n_components=2, n_jobs=4, verbose=0, n_iter=2000),
            ...
            filename = os.path.join(
                out_dir, split + "_" + str(key) + "_" + visualization_name + ".png"
            )
...
def post_processing(post_processing, results, ckpt_dir, seed, dl, **kwargs):
    ...
    for pp in post_processing:
        if "plot_attention" in pp:  # Plot attention weights
            plot_attention(
                results=results, pp_dir=pp_dir, seed=seed, logger=logger, **kwargs
            )
